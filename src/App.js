...
const sumH = m   => m.tekijaTunnit ? Object.values(m.tekijaTunnit).reduce((a,v)=>a+Number(v||0),0) : Number(m.tunnit||0);
...
async function doPDF(m) {
  ...
  doc.setFontSize(9);  doc.text("TYÖMÄÄRÄIN", L, 17);
  ...
  const row=(lbl,val,bold=false)=>{
    ...
    doc.text(lbl.toUpperCase(), L, y);
    doc.setFont("helvetica",bold?"bold":"normal"); doc.setFontSize(10); doc.setTextColor(20,20,20);
    const lines=doc.splitTextToSize(String(val||"-"),W-L*2);
    doc.text(lines,L,y+5); y+=6+lines.length*5+4;
    doc.setDrawColor(220,220,220); doc.line(L,y-2,W-L,y-2);
  };
  row("Kone / Laite", m.kone, true);
  if (m.tekijaTunnit) row("Tekijät & tunnit", Object.entries(m.tekijaTunnit).map(([k,v])=>`${k}: ${v}h`).join("  |  ")+" → Yht. "+sumH(m)+"h");
  row("Konetunnit (mittarilukema)", (m.konetunnit||"?") + " h");
  row("Päivämäärä", fd(m.pvm));
  row("Mitä tehty", m.kuvaus);
  if (m.lisatiedot) row("Lisätiedot", m.lisatiedot);
...
function doCSV(list) {
  ...
  const H=["ID","Kone","Päivämäärä","Konetunnit (h)","Työtunnit","Status","Kuvaus","Lisätiedot","Luotu",...tlist.map(t=>t+" (h)")];
  const rows=list.map(m=>{
    ...
    tlist.forEach(t=>r.push(m.tekijaTunnit?m.tekijaTunnit[t]||"":""));
    return r;
  });
...
// ── Firestore helpers ─────────────────────────────────────────────────────────
const SETTINGS_DOC = "asetukset/data";

async function loadSettings() {
  try {
    const snap = await getDoc(doc(db, "asetukset", "data"));
...
export default function App() {
  ...
  // Realtime listener: työmääräimet
  useEffect(() => {
  ...
  const addWO = async (data, addKone) => {
    const id = uid();
    try {
      await setDoc(doc(db, "tyomaaraimet", id), {...data, id, luotu: new Date().toISOString()});
    } catch(e) { console.error(e); }
    if (addKone && !koneet.includes(addKone)) {
      const base = koneet.filter(k=>k!=="Muu kone");
      const next = [...base, addKone, "Muu kone"];
  ...
  return (
    <div style={R.root}>
      <div style={R.header}>
        <div style={R.htop}>
          <div>
            <div style={R.logo}>⚙ FANTUZZI FIN</div>
            <div style={R.sub}>Työmääräinjärjestelmä</div>
          </div>
      ...
      <div style={R.body}>
        ...
        {filtered.map(w=>{
          ...
          const ts=w.tekijaTunnit?Object.entries(w.tekijaTunnit).map(([k,v])=>`${k} ${v}h`).join(", "):"";
          return (
            <div key={w.id} style={R.card} onClick={()=>{setSel(w);setView("detail");}}>
              ...
              <div style={{fontWeight:700,fontSize:14,color:"#111827",marginBottom:4}}>{w.kone}</div>
              <div style={{fontSize:12,color:"#6b7280",marginBottom:8,overflow:"hidden",textOverflow:"ellipsis",display:"-webkit-box",WebkitLineClamp:2,WebkitBoxOrient:"vertical"}}>{w.kuvaus}</div>
...
function NewForm({koneet, tekijat, onSave, onBack}) {
  ...
  const handleSave = async (addKone) => {
    setSaving(true);
    await onSave({kone:kNimi,tekijaTunnit:ttmap,pvm,konetunnit,kuvaus,lisatiedot:lisat,status}, addKone||null);
  };
      ...
      <div style={R.body}>
        <div style={{...R.card,background:"#fffbeb",border:"1px solid #fde68a"}}>
          <div style={{fontWeight:700,fontSize:14,color:"#1f2937",marginBottom:8}}>
            Lisätäänkö "{muuNimi.trim()}" konelistan?
          </div>
  ...
  return (
    <div style={R.root}>
      <div style={R.header}>
        <div style={R.htop}><div style={R.logo}>UUSI TYÖMÄÄRÄIN</div><Btn onClick={onBack}>← Takaisin</Btn></div>
      </div>
      <div style={R.body}>
...
function Detail({w, onBack, onStatus, onDelete, onEdit}) {
  ...
  const s   = wos(w.status);
  const tot = sumH(w);
  const entries = w.tekijaTunnit ? Object.entries(w.tekijaTunnit) : [];
    ...
    <div style={R.root}>
      ...
      <div style={R.body}>
        <div style={R.card}>
          ...
          <Sec label="MITÄ TEHTY">{w.kuvaus}</Sec>
          ...
          <div style={R.tbox}>
            {entries.map(([t,h])=>(
              <div key={t} style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
                <span style={{fontSize:13,color:"#374151"}}>👤 {t}</span>
                <span style={{fontWeight:700,color:"#d97706"}}>{h} h</span>
              </div>
            ))}
            {entries.length>1&&(
              <div style={{display:"flex",justifyContent:"space-between",borderTop:"1px solid #e5e7eb",paddingTop:8,marginTop:4}}>
...
function KoneStatus({koneet, kstat, onSet, onBack}) {
  ...
  const openKone = k => {
    const cur = kstat[k];
    setOpen(k); setSid(cur?.status||"ok"); setNote(cur?.note||""); setSaved(false);
  };

  const save = async () => {
    await onSet(open, sid, note);
    setSaved(true); setTimeout(()=>{setSaved(false);setOpen(null);},1200);
  };

  return (
    <div style={R.root}>
      <div style={R.header}>
        <div style={R.htop}><div style={R.logo}>📊 KONESTATUS</div><Btn onClick={onBack}>← Takaisin</Btn></div>
      </div>
      <div style={R.body}>
...
function Settings({koneet, tekijat, onSave, onBack}) {
  ...
  return (
    <div style={R.root}>
      <div style={R.header}>
        <div style={R.htop}><div style={R.logo}>⚙ ASETUKSET</div><Btn onClick={onBack}>← Takaisin</Btn></div>
      </div>
      <div style={R.body}>
        <div style={R.card}>
          ...
          <div style={{marginTop:8,marginBottom:20}}>
            {kl.map((k,i)=>(
              <div key={i} style={{display:"flex",gap:8,marginBottom:6}}>
                <input style={{...R.input,flex:1,marginBottom:0}} value={k}
                  onChange={e=>setKl(l=>l.map((x,j)=>j===i?e.target.value:x))}/>
                <button style={R.xBtn} onClick={()=>setKl(l=>l.filter((_,j)=>j!==i))}>✕</button>
              </div>
            ))}
            <div style={{display:"flex",gap:8}}>
              <input style={{...R.input,flex:1,marginBottom:0}} placeholder="Lisää kone..."
                value={nk} onChange={e=>setNk(e.target.value)} onKeyDown={e=>e.key==="Enter"&&addK()}/>
          ...
          <div style={{marginTop:8,marginBottom:24}}>
            {tl.map((t,i)=>(
              <div key={i} style={{display:"flex",gap:8,marginBottom:6}}>
                <input style={{...R.input,flex:1,marginBottom:0}} value={t}
                  onChange={e=>setTl(l=>l.map((x,j)=>j===i?e.target.value:x))}/>
                <button style={R.xBtn} onClick={()=>setTl(l=>l.filter((_,j)=>j!==i))}>✕</button>
              </div>
            ))}
            <div style={{display:"flex",gap:8}}>
              <input style={{...R.input,flex:1,marginBottom:0}} placeholder="Lisää tekijä..."
                value={nt} onChange={e=>setNt(e.target.value)} onKeyDown={e=>e.key==="Enter"&&addT()}/>
...
function Btn({children, onClick, primary, icon, full, disabled}) {
  return (
    <button onClick={onClick} disabled={disabled} style={{
      background: primary?"#d97706":"#f9fafb",
      color: primary?"#fff":"#6b7280",
      border: primary?"none":"1px solid #e5e7eb",
      borderRadius:6, padding: icon?"6px 10px":full?"14px":"7px 14px",
      fontSize: full?14:12, fontWeight:700,
...
function Label({children}) {
  return <div style={{fontSize:10,color:"#9ca3af",letterSpacing:1.5,fontFamily:"monospace",marginBottom:4,marginTop:14,textTransform:"uppercase"}}>{children}</div>;
}
function Hint({children, red}) {
  return <div style={{fontSize:10,color:red?"#dc2626":"#9ca3af",marginBottom:6}}>{children}</div>;
}
function Sec({label,children}) {
  return <div style={{marginBottom:14}}>
    <div style={R.lbl}>{label}</div>
    <div style={{fontSize:14,color:"#374151",lineHeight:1.5,marginTop:4}}>{children}</div>
  </div>;
}
...
const R = {
  ...
  logo:   {fontSize:15,fontWeight:700,color:"#d97706",letterSpacing:2},
  sub:    {fontSize:10,color:"#9ca3af",letterSpacing:1,marginTop:2},
  body:   {padding:16},
  ...
  search: {width:"100%",background:"#fff",border:"1px solid #e5e7eb",borderRadius:8,color:"#1f2937",padding:"10px 12px",fontSize:13,fontFamily:"'Courier New',monospace",marginBottom:14,boxSizing:"border-box",boxShadow:"0 1px 3px #00000008"},
  card:   {background:"#fff",border:"1px solid #e5e7eb",borderRadius:10,padding:16,marginBottom:10,boxShadow:"0 1px 4px #00000008"},
  badge:  {fontSize:9,fontWeight:700,padding:"3px 8px",borderRadius:10,letterSpacing:0.5,whiteSpace:"nowrap"},
  tbox:   {background:"#f9fafb",border:"1px solid #e5e7eb",borderRadius:8,padding:"10px 14px",marginBottom:4},
  lbl:    {fontSize:9,color:"#9ca3af",letterSpacing:2,textTransform:"uppercase",marginBottom:4},
  input:  {width:"100%",background:"#f9fafb",border:"1px solid #e5e7eb",borderRadius:6,color:"#1f2937",padding:"10px 12px",fontSize:13,fontFamily:"'Courier New',monospace",boxSizing:"border-box",outline:"none",marginBottom:6},
  stBtn:  {background:"transparent",border:"1px solid",borderRadius:6,padding:"6px 12px",fontSize:11,fontWeight:700,fontFamily:"'Courier New',monospace",cursor:"pointer"},
