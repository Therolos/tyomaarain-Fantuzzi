...
const WO_STATUS = [
  { id:"avoin",   label:"Avoin",        c:"#d97706" },import { useState, useEffect } from "react";
import { collection, doc, onSnapshot, setDoc, deleteDoc, getDoc } from "firebase/firestore";
import { db } from "./firebase";
  ...
  return(
    <div style={R.root}>
      ...
      <div style={R.body}>
        ...
        <GroupedKoneSelect koneet={koneet} value={kone} onChange={v=>{setKone(v);setKysy(false);}}/>
        {isMuu&&<>
...
// ── GroupedKoneSelect ─────────────────────────────────────────────────────────
function GroupedKoneSelect({koneet, value, onChange}) {
  // Ryhmittele viimeisen sanan mukaan
  const groups = {};
  koneet.forEach(k => {
    const parts = k.trim().split(/\s+/);
    const last = parts.length > 1 ? parts[parts.length-1] : "Muut";
    const group = last === "kone" ? "Muut" : last;
    ...
    <select style={R.input} value={value} onChange={e=>onChange(e.target.value)}>
      {sortedGroups.map(group => (
        <optgroup key={group} label={`── ${group} ──`}>
          {groups[group].map(k => <option key={k}>{k}</option>)}
        </optgroup>
      ))}
    </select>
  );
